...
func (r *soalRepositoryImpl) GetByID(id int) (*entity.Soal, error) {
	...
	err := r.db.Preload("Materi").Preload("Materi.MataPelajaran").First(&soal, id).Error
	if err != nil {
		return nil, err
	}
	return &soal, nil
...
func (r *soalRepositoryImpl) List(idMateri, tingkatan, idMataPelajaran *int, limit, offset int) ([]entity.Soal, int, error) {
	...
	query := r.db.Model(&entity.Soal{}).Preload("Materi").Preload("Materi.MataPelajaran")
...
func (r *soalRepositoryImpl) GetByMateriID(idMateri int) ([]entity.Soal, error) {
	var soals []entity.Soal
	err := r.db.Preload("Materi").Preload("Materi.MataPelajaran").Where("id_materi = ?", idMateri).Find(&soals).Error
	return soals, err
}